...
def get_random() -> float:
    """
    Fetches a random float between 0.00 and 0.99 (2 decimal places) from random.org.

    Returns:
        float: A random float between 0.00 and 0.99
    ...
    """
    try:
        logger.info(f"Fetching random float from {RANDOM_ORG_URL}")
        ...
        logger.info(f"Received random floar: {random_number}")
